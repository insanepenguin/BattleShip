...
{//open class
   ...
   {//open constructor
      ...
      try {
         playersReady[0].join();
         playersReady[1].join();
         new Connection(sockets[0], pOne.getName(), goesFirst).start();
         new Connection(sockets[1], pTwo.getName(), goesFirst).start();
      }
      ...
      public void run() {
         ...
         {//open try
            ...
            {//open if                                         if it is, make our Ship array equal to the incoming object casted as a ship array
               ...
               //TEST SHIT HERE
                pOne = new Player(player1Field,17,"1");
               pTwo = new Player(player2Field,17,"1");
   ...
   public class Connection extends Thread {
     ...
     int first;
      ...
      public Connection(Socket _cs, String _playerNum, int _first) {
            ...
            try {
               ...
               console(getName() + "initialized");
                  ...
                  while(sock.isConnected()) {
                     ...
                     boolean hitMiss = pOne.turn(xCoord,yCoord);

                     synchronized(sync) {
                        currentMsg = playerNum + "," + hitMiss + "," + xCoord + "," + yCoord;
                        sync.notifyAll();
                     }
                     if(pOne.loser || pTwo.loser) {
                        if(pOne.loser) {
